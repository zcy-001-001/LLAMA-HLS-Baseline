...
int main(int argc, char *argv[]) {
    ...
    std::string checkpoint_path =model_base_path+ "/llama2_7b_s8g64.bin"; // Default for CSIM
    ...
    char *prompt = "Once";    // Default prompt is NULL (empty -> starts with BOS)
    ...
    // --- Build Transformer ---
    // MODIFICATION: Changed from a static object to a dynamically allocated pointer.
    // This moves the large memory allocation from the static data segment to the heap,
    // avoiding the linker's 2GB limit.
    Transformer<dim, hidden_dim, n_layers, n_heads, n_kv_heads,
                vocab_size, seq_len, GS> *transformer;
    
    try {
        transformer = new Transformer<dim, hidden_dim, n_layers, n_heads, n_kv_heads,
                                      vocab_size, seq_len, GS>();
    } catch (const std::bad_alloc& e) {
        fprintf(stderr, "ERROR: Memory allocation for Transformer failed: %s\n", e.what());
        return 1;
    }


    // MODIFICATION: When passing 'transformer' to functions, it's already a pointer,
    // so we no longer need the address-of operator '&'.
    ...
    if (!weights_loaded) {
        ...
        delete transformer; // Clean up memory on error
        return 1;
    }

     // MODIFICATION: Use the '->' operator to access members of a pointer to a struct/class.
    ...
    // --- Build Tokenizer ---
    Tokenizer tokenizer;
    // MODIFICATION: Use '->' operator here as well.
    ...
    if (strcmp(mode, "generate") == 0) {
        // MODIFICATION: Pass the pointer 'transformer' directly.
    ...
    // MODIFICATION: Since we allocated with 'new', we MUST free the memory with 'delete'.
    delete transformer;
    transformer = nullptr; // Good practice to nullify pointer after deletion.
